...
export async function createSession(sessionId: string): Promise<SessionState> {
  if (isMemoryStorage()) return memory.createSession(sessionId);
...
export async function getSession(sessionId: string): Promise<SessionState | null> {
  if (isMemoryStorage()) return memory.getSession(sessionId);
...
export async function saveSession(session: SessionState): Promise<SessionState> {
  if (isMemoryStorage()) return memory.saveSession(session);
...
export async function getBooking(id: string): Promise<Booking | null> {
  if (isMemoryStorage()) return memory.getBooking(id);
  const row = await prisma.booking.findUnique({ where: { id } });
  return row ? rowToBooking(row) : null;
}

export async function listBookings(): Promise<Booking[]> {
  if (isMemoryStorage()) return memory.listBookings();
...
export async function saveBooking(booking: Booking): Promise<Booking> {
  if (isMemoryStorage()) return memory.saveBooking(booking);
...
export async function getDashboardPatient(id: number): Promise<DashboardPatient | null> {
  if (isMemoryStorage()) return memory.getDashboardPatient(id);
...
export async function listDashboardPatients(): Promise<DashboardPatient[]> {
  if (isMemoryStorage()) return memory.listDashboardPatients();
...
export async function getPatientIdForBooking(bookingId: string): Promise<number | null> {
  if (isMemoryStorage()) return memory.getPatientIdForBooking(bookingId);
...
export async function getBookingIdForPatient(patientId: number): Promise<string | null> {
  if (isMemoryStorage()) return memory.getBookingIdForPatient(patientId);
  const row = await prisma.dashboardPatient.findUnique({
    where: { id: patientId },
...
export async function upsertDashboardPatient(
  ...
  if (isMemoryStorage()) return memory.upsertDashboardPatient(bookingId, patient);
  const data = {
    name: patient.name,
    age: patient.age,
...
export async function updateDashboardPatientStatus(
  ...
  if (isMemoryStorage()) return memory.updateDashboardPatientStatus(patientId, status);
...
export async function getThread(patientId: number): Promise<DashboardThread | null> {
  if (isMemoryStorage()) return memory.getThread(patientId);
...
export async function listThreadSummaries() {
  if (isMemoryStorage()) return memory.listThreadSummaries();
  ...
  return threads.map((th) => ({
    ...
    unread: th.unread,
    last: th.messages[0]
      ? { who: th.messages[0].who as "me" | "them", t: th.messages[0].t, txt: th.messages[0].txt }
      : null
  }));
}

export async function ensureThread(patientId: number): Promise<DashboardThread> {
  if (isMemoryStorage()) return memory.ensureThread(patientId);
...
export async function appendThreadMessage(
  ...
  if (isMemoryStorage()) return memory.appendThreadMessage(patientId, msg, opts);
...
export async function markThreadRead(patientId: number): Promise<DashboardThread> {
  if (isMemoryStorage()) return memory.markThreadRead(patientId);
  await prisma.dashboardThread.update({
    where: { patientId },
    data: { unread: false }
  });
...
export async function countUnreadThreads(): Promise<number> {
  if (isMemoryStorage()) return memory.countUnreadThreads();
...
export async function getDashboardProfile(): Promise<DashboardProfile> {
  if (isMemoryStorage()) return memory.getDashboardProfile();
...
export async function updateDashboardProfile(
  ...
  if (isMemoryStorage()) return memory.updateDashboardProfile(patch);
  const row = await prisma.dashboardProfile.upsert({
    ...
    create: {
      ...
      whatsappNotifications:
        patch.whatsapp_notifications ?? DEFAULT_PROFILE.whatsapp_notifications
    },
...
export async function getDashboardRuntime(): Promise<{
  ...
  if (isMemoryStorage()) return memory.getDashboardRuntime();
...
export async function saveDashboardRuntime(data: {
  ...
  if (isMemoryStorage()) return memory.saveDashboardRuntime(data);
...
export async function seedDatabase(): Promise<void> {
  if (isMemoryStorage()) return memory.seedMemoryStore();
