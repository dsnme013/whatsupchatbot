...
import type { TriageInput } from "./lib/triage.js";
...
import {
  ...
  setOpenSlots,
  setSchedule
} from "./db/runtimeState.js";
import type { Booking, DashboardPatient, DashboardProfile } from "./store.js";
...
const normalizeSlotLabel = (label: string) => label.trim().replace(/\s+/g, " ").toUpperCase();

const dashboardModeToBooking = (mode: DashboardPatient["mode"]): "video" | "home" =>
  mode === "Video" ? "video" : "home";

/** Resolve a dashboard slot label (e.g. "7:00 PM") to ISO + mode — including ad-hoc open slots. */
function resolveSlotFromLabel(
  slotLabel: string,
  doc: (typeof doctors)[0],
  opts: { referenceIso?: string; preferredMode: "video" | "home" }
): { atIso: string; mode: "video" | "home" } {
...
const onsetLabel = (onset?: TriageInput["onset"]) => {
  switch (onset) {
    case "today":
      return "Today";
    case "yesterday":
...
const redFlagLabels = (input?: TriageInput) => {
  if (!input?.redFlags) return [];
  const f = input.redFlags;
  const out: string[] = [];
  if (f.chestPain) out.push("Chest pain");
  if (f.breathingTrouble) out.push("Breathing trouble");
  if (f.fainting) out.push("Fainting");
  if (f.confusion) out.push("Confusion");
  if (f.severeWeakness) out.push("Severe weakness");
  if (f.stiffNeck) out.push("Stiff neck");
  if (f.severeDehydration) out.push("Dehydration");
  if (f.spo2Below92) out.push("Low SpO2");
  return out;
};

function dashboardGender(gender?: "male" | "female" | "other"): "M" | "F" | "O" {
  if (gender === "male") return "M";
  if (gender === "female") return "F";
  return "O";
}

function bookingFeeInr(booking: Booking): number {
...
async function refreshAllBookingFees() {
  ...
  for (const booking of all) {
    ...
    if (pid != null) await upsertDashboardFromBooking(updated);
  }
}

async function totalEarningsInr(): Promise<number> {
  const patients = await repo.listDashboardPatients();
  return patients.reduce((sum, p) => sum + p.fee, 0);
}

async function bookingToPatient(booking: Booking, patientId: number): Promise<DashboardPatient> {
...
async function refreshScheduleFromBookings() {
  const slots: ReturnType<typeof getSchedule> = [];
  const open: string[] = [];
  const now = Date.now();
  let nearestIdx = -1;
  ...
  for (const doc of doctors) {
    for (const s of doc.nextSlots) {
      ...
      const booked = allBookings.find(
        (b) => b.doctorId === doc.id && b.slotAtIso === s.atIso
      );
      ...
      if (pid == null) open.push(label);
    }
  }

  if (nearestIdx >= 0) slots[nearestIdx] = { ...slots[nearestIdx], now: true };
  setSchedule(slots);
  setOpenSlots(open);
...
export async function upsertDashboardFromBooking(booking: Booking) {
  ...
  const symptomNote =
    triage?.summaryForDoctor.oneLiner ??
  ...
  if (symptomNote && !thread) {
    await repo.appendThreadMessage(
      saved.id,
      {
        who: "them",
...
async function patientOr404(pid: number): Promise<DashboardPatient> {
  const p = await repo.getDashboardPatient(pid);
  if (!p) throw Object.assign(new Error("patient_not_found"), { status: 404 });
  return p;
}

function formatPatientAddress(patient: Booking["patient"]): string | undefined {
  if (patient.address?.trim()) return patient.address.trim();
  const parts = [patient.house_number, patient.village, patient.city].filter(Boolean);
  if (!parts.length && !patient.pincode) return undefined;
  const line = parts.join(", ");
  return patient.pincode ? `${line} - ${patient.pincode}` : line;
}

function buildApprovalMessage(booking: Booking, doctorName: string, timeLabel: string) {
...
function buildRescheduleMessage(booking: Booking, doctorName: string, whenLabel: string) {
  const mode = booking.mode === "video" ? "Video consult" : "Home visit";
  const ref = `CC-${booking.id.toUpperCase()}`;
  const fee = bookingFeeInr(booking);
...
async function notifyPatientApproval(pid: number, patient: DashboardPatient) {
...
async function updateBookingFromPatient(patient: DashboardPatient) {
...
async function notifyPatientReschedule(pid: number, _patient: DashboardPatient, booking: Booking) {
  const doc = doctors.find((d) => d.id === booking.doctorId);
  const doctorName = doc?.name ?? getDashboardProfileSync().name;
...
export async function registerDoctorRoutes(app: FastifyInstance) {
  ...
  app.get("/api/v1/queue", async (req) => {
    const { tier } = req.query as { tier?: "low" | "moderate" | "high" };
  ...
  app.post("/api/v1/booking/:id/accept", async (req, reply) => {
    ...
    if (!Number.isFinite(pid)) return reply.code(400).send({ detail: "invalid_id" });
    let p: DashboardPatient;
    try {
      p = await patientOr404(pid);
    } catch {
      return reply.code(404).send({ detail: "patient_not_found" });
    }
    if (p.status === "accepted") return reply.code(409).send({ detail: "already_accepted" });
    p = { ...p, status: "accepted" };
    const bookingId = await repo.getBookingIdForPatient(pid);
    if (bookingId) {
  ...
  app.patch("/api/v1/booking/:id/reschedule", async (req, reply) => {
    ...
    const body = req.body as { slot?: string; slotAtIso?: string; mode?: "video" | "home" };

    // Doctor dashboard: numeric patient id + { slot: "1:00 PM" }
    if (/^\d+$/.test(id)) {
      const pid = Number(id);
      const { slot } = body;
      if (!slot?.trim()) return reply.code(400).send({ detail: "slot_required" });
      ...
      const bookingId = await repo.getBookingIdForPatient(pid);
      if (bookingId) await repo.upsertDashboardPatient(bookingId, p);
      ...
      replaceOpenSlots((slots) => {
        ...
        if (!next.includes(oldTime)) next.push(oldTime);
        return next;
      });
      await persistRuntimeState();
    ...
    let slotMatch = doc.nextSlots.find(
      (sl) => sl.atIso === parsed.data.slotAtIso && sl.mode === parsed.data.mode
    );
    if (!slotMatch) slotMatch = doc.nextSlots.find((sl) => sl.mode === parsed.data.mode);
    if (!slotMatch) slotMatch = doc.nextSlots.find((sl) => sl.atIso === parsed.data.slotAtIso);
    if (!slotMatch && doc.nextSlots.length > 0) slotMatch = doc.nextSlots[0];
    if (!slotMatch)
  ...
  app.post("/api/v1/schedule/slot", async (req, reply) => {
    ...
    if (!t?.trim()) return reply.code(400).send({ detail: "t_required" });
  ...
  app.get("/api/v1/patients/:id/record", async (req, reply) => {
    ...
    const nextStepLabel = (step?: string) => {
      switch (step) {
  ...
  app.get("/api/v1/messages/:pid", async (req, reply) => {
    ...
    if (!th) return reply.code(404).send({ detail: "thread_not_found" });
  ...
  app.post("/api/v1/messages/:pid", async (req, reply) => {
    ...
    if (!text?.trim()) return reply.code(400).send({ detail: "text_required" });
    ...
    return {
      summary: {
        ...
        completion_pct: patients.length
          ? Math.round(
              (patients.filter((p) => p.status === "accepted").length / patients.length) * 100
            )
          : 0,
        no_shows: 0,
        avg_consult_min: 14,
      ...
      triage_mix: (["high", "moderate", "low"] as const).map((tier) => ({
        tier,
        n: byTier[tier],
  ...
  app.put("/api/v1/profile", async (req) => {
    const body = req.body as Partial<DashboardProfile>;
    ...
    if (patch.home_fee != null) patch.home_fee = Number(patch.home_fee);
  ...
  app.post("/api/v1/doctor/booking/:id/accept", async (req, reply) => {
    ...
    if (pid == null) return reply.code(404).send({ error: "booking_not_found" });
    const booking = await repo.getBooking(id);
    if (!booking) return reply.code(404).send({ error: "booking_not_found" });
    if (booking.status === "accepted")
      return reply.code(409).send({ error: "already_accepted" });
    const p = await repo.getDashboardPatient(pid);
    if (!p) return reply.code(404).send({ error: "patient_not_found" });
  ...
  app.patch("/api/v1/doctor/booking/:id/reschedule", async (req, reply) => {
    const { id } = req.params as { id: string };
    const { slotAtIso } = req.body as { slotAtIso?: string };
    const booking = await repo.getBooking(id);
    if (!booking) return reply.code(404).send({ error: "booking_not_found" });
    if (!slotAtIso)
    ...
    if (!slot) return reply.code(409).send({ error: "slot_unavailable" });
