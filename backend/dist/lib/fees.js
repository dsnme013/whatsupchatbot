...
export function resolveConsultationFee(doctorId, mode) {
    if (doctorId === CONSOLE_DOCTOR_ID)
        return profileFee(mode);
    const doc = doctors.find((d) => d.id === doctorId);
    if (doc)
        return doc.price[mode];
    return profileFee(mode);
}
export function applyLivePrices(doc) {
    return {
        ...doc,
        price: {
