...
import { ConsentSchema, CreateBookingSchema, DoctorMatchRequestSchema, HistorySchema, RescheduleBookingSchema, TriageRequestSchema } from "./schemas.js";
...
function rankDoctors(args) {
    ...
    const filtered = doctors
        ...
        if (specText.includes("fever") && symptom.includes("fever"))
            fit += 18;
        if (specText.includes("headache") && symptom.includes("head"))
            fit += 18;
        if (specText.includes("infection") && (symptom.includes("fever") || symptom.includes("cold") || symptom.includes("cough")))
            fit += 10;
        if (comorb.includes("bp") || comorb.includes("hypertension")) {
            if (specText.includes("internal medicine") || specText.includes("chronic"))
                fit += 12;
        }
    ...
    if (sortBy === "soonest")
        filtered.sort((a, b) => a.soonestAtMs - b.soonestAtMs);
    else if (sortBy === "top_rated")
        filtered.sort((a, b) => b.doctor.rating - a.doctor.rating);
    else if (sortBy === "fastest_response")
        filtered.sort((a, b) => a.doctor.responseTimeMinsP50 - b.doctor.responseTimeMinsP50);
    else
...
export async function registerRoutes(app) {
    ...
    app.post("/api/v1/booking", async (req, reply) => {
        ...
        let slot = doc.nextSlots.find((sl) => sl.atIso === parsed.data.slotAtIso && sl.mode === parsed.data.mode);
        if (!slot) {
        ...
        if (!slot)
            return reply.code(409).send({ error: "slot_unavailable", message: "No slots available for this doctor." });
        const id = nanoid(10);
        const booking = {
            id,
